                            ...
                            if ($taxonomy->name === $active_filter['type']) {
                                ...
                                if($term_query['taxonomy']==='product_cat') {
                                    if(isset($query_vars['__wholesalex_exclude_cat']) && !empty($query_vars['__wholesalex_exclude_cat'])) {
                                        $term_query['exclude'] = $query_vars['__wholesalex_exclude_cat'];
                                    }
                                ...
                                if(isset($query_vars['post__not_in']) && !empty($query_vars['post__not_in'])) {
                                    $attr['post__not_in'] = $query_vars['post__not_in'];
...
?>
        <?php
            ...
            foreach ($taxonomy->terms as $term) {
                ...
                if(isset($query_vars['post__not_in']) && !empty($query_vars['post__not_in'])) {
                    $query_args['post__not_in'] = $query_vars['post__not_in'];
        ...
        ?>
            <div class="wopb-filter-check-item-section <?php esc_attr_e($extended_item_class) ?>"
                 data-hidden-term-count="<?php esc_attr_e($params['hiddenTermCount']) ?>">
                <div class="wopb-filter-check-item">
...
<?php
    ...
    public function wopb_show_more_filter_item_callback() {
        ...
        $params = [
            ...
            'hiddenTermCount' => $_POST['hiddenTermCount'],
        ];
        $term_query = array (
            'taxonomy' => $taxonomy->name,
            'hide_empty' => true,
        ...
        $term_offset = ($_POST['item_page'] - 1) * $_POST['term_limit'];
