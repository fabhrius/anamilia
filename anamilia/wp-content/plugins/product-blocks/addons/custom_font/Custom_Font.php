...
class Custom_Font {
    ...
    public function templates_table_content( $column_id, $post_id ) {
        ...
        if ($settings) {
            ...
            switch ($column_id) {
                case 0:
                    echo '<span class="wopb-custom-font-preview" style="font-family: '.get_the_title($post_id).'">' . __('The quick brown fox jumps over the lazy dog.', 'product-blocks') . '</span>';
                    break;
                case 1:
    ...
    function set_data($arr = [], $font_name='') { ?>
        <div class="wopb-custom-font-container wopb-custom-font<?php echo empty($arr) ? '-copy' : ''; ?>">
            <div class="wopb-custom-font-heading">
                <div>
                    <label class="font-label"><?php _e('Weight:  ', 'product-blocks'); ?> <span class="wopb-custom-font-weight"> <?php _e(isset($arr['weight']) ? $arr['weight'] : '', 'product-blocks'); ?> </span></label>
                    <select name="weight[]">
                        <?php $weight = isset($arr['weight']) ? $arr['weight'] : ''; ?>
                        <option <?php selected( $weight, 'normal' ); ?> value="normal"><?php _e('Normal', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '100' ); ?> value="100"><?php _e('100', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '200' ); ?> value="200"><?php _e('200', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '300' ); ?> value="300"><?php _e('300', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '400' ); ?> value="400"><?php _e('400', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '500' ); ?> value="500"><?php _e('500', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '600' ); ?> value="600"><?php _e('600', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '700' ); ?> value="700"><?php _e('700', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '800' ); ?> value="800"><?php _e('800', 'product-blocks'); ?></option>
                        <option <?php selected( $weight, '900' ); ?> value="900"><?php _e('900', 'product-blocks'); ?></option>
                    </select>
                </div>
                <?php
                    $styles = '';
                ...
                ?>
                <span <?php echo $styles; ?> class="wopb-custom-font-preview"><?php _e('The quick brown fox jumps over the lazy dog', 'product-blocks'); ?></span>
                <div class="wopb-custom-font-actions">
                    <span class="wopb-custom-font-edit"><span class="dashicons dashicons-edit"></span><?php _e('Edit', 'product-blocks'); ?></span>
                    <span class="wopb-custom-font-close"><span class="dashicons dashicons-no-alt"></span><?php _e('Close', 'product-blocks'); ?></span>
                    <span class="wopb-custom-font-delete"><span class="dashicons dashicons-trash"></span><?php _e('Delete', 'product-blocks'); ?></span>
                </div>
            </div>
            <div class="wopb-custom-font-content">
                <div class="wopb-font-file-list">
                    <label><?php _e('WOFF File', 'product-blocks'); ?></label>
                    <input type="text" name="woff[]" value="<?php echo isset($arr['woff']) ? $arr['woff'] : ''; ?>" placeholder="<?php _e('The web open Font Format, Used by Modern Browsers', 'product-blocks'); ?>"/>
                    <span class="button wopb-font-upload"><span class="dashicons dashicons-upload"></span><?php _e('Upload', 'product-blocks'); ?></span>
                </div>
                <div class="wopb-font-file-list">
                    <label><?php _e('WOFF2 File', 'product-blocks'); ?></label>
                    <input type="text" name="woff2[]" value="<?php echo isset($arr['woff2']) ? $arr['woff2'] : ''; ?>" placeholder="<?php _e('The web open Font Format 2, Used by Super Modern Browsers', 'product-blocks'); ?>"/>
                    <span class="button wopb-font-upload"><span class="dashicons dashicons-upload"></span><?php _e('Upload', 'product-blocks'); ?></span>
                </div>
                <div class="wopb-font-file-list">
                    <label><?php _e('TTF File', 'product-blocks'); ?></label>
                    <input type="text" name="ttf[]" value="<?php echo isset($arr['ttf']) ? $arr['ttf'] : ''; ?>" placeholder="<?php _e('TrueType Fonts, Used for better supporting Safari, Android, iOS', 'product-blocks'); ?>"/>
                    <span class="button wopb-font-upload"><span class="dashicons dashicons-upload"></span><?php _e('Upload', 'product-blocks'); ?></span>
                </div>
                <div class="wopb-font-file-list">
                    <label><?php _e('SVG File', 'product-blocks'); ?></label>
                    <input type="text" name="svg[]" value="<?php echo isset($arr['svg']) ? $arr['svg'] : ''; ?>" placeholder="<?php _e('SVG font allow SVG to be used as glyphs when displaying text, Used by Legacy iOS', 'product-blocks'); ?>"/>
                    <span class="button wopb-font-upload"><span class="dashicons dashicons-upload"></span><?php _e('Upload', 'product-blocks'); ?></span>
                </div>
                <div class="wopb-font-file-list">
                    <label><?php _e('EOT File', 'product-blocks'); ?></label>
                    <input type="text" name="eot[]" value="<?php echo isset($arr['eot']) ? $arr['eot'] : ''; ?>" placeholder="<?php _e('Embedded OpenType, Used by IE6-IE9 Browsers', 'product-blocks'); ?>"/>
                    <span class="button wopb-font-upload"><span class="dashicons dashicons-upload"></span><?php _e('Upload', 'product-blocks'); ?></span>
                </div>
            </div>
        </div>
    ...
    function custom_font_callback($post) {
        ...
        echo '<span class="button wopb-font-variation-action">'.__('Add Variation', '').'</span>';
    }
