                ...
                foreach ($terms as $term) {
                    ...
                    if (in_array($term->slug, $options)) {
                        ...
                        foreach ($product->get_available_variations() as $available_variation) {
                            ...
                            if(in_array($term->slug, $variation_attributes_values)) {
                                $data_variation_id = $available_variation['variation_id'];
                                if (($taxonomy_attribute->attribute_type == self::COLOR || $taxonomy_attribute->attribute_type == self::IMAGE) && $variation->get_image_id('edit') != 0) {
                                    $variation_image = $available_variation['image']['thumb_src'];
                                    break;
                                }
                            }
                        }
                        if(count($product->get_attributes()) == 1 && !$data_variation_id) {
                            continue;
                        }
